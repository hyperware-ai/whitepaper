...
The \verb|on_exit| field sets the behavior of the process when it exits:

There are three possible behaviors.

- 1. \verb|"None"| - The process is not restarted and nothing happens.

- 2. \verb|"Restart"| - The process is restarted immediately.

- 3. \verb|"Requests"| - The process is not restarted, and a list of requests set by the process are fired off. These requests have the \verb|source| and \verb|capabilities| of the exiting process.
...
Binding \$KINO to packages published in kimap allows users to (a) only display apps that have a certain amount of value assigned to them, filtering out discarded tests and spam, and (b) have a metric to compare apps against one another, allowing one to compare two similarly-named apps and easily see which is more widely adopted in the peer-to-peer network.
...
Each entry may also be minted as an ERC-6551 token-bound account.
...
\begin{enumerate}
    ...
    \item Every name-key is an ERC-721 NFT with an ERC-6551 token-bound account.
        \subitem Name-keys may create sub-entries directly beneath themselves
        \subitem Name-keys may inscribe data in data-keys directly beneath themselves.
    ...
    \item The top level keys are called Top-Level Zones or TLZs.
        \subitem TLZ minting will be governed in a decentralized manner.
        \subitem Once created, a TLZ may define custom rules for its sub-entries.
        \subitem TLZs will produce the tapestry of namespace governance schemes that allow kimap to be used for a wide variety of protocols.
\end{enumerate}
...
As noted above, the binding protocol is quite simple.
More complex elements such as onchain yield, however, are likely to be built by others on top of kimap+KINO.
...
Phase 2 will last until EITHER:
\begin{itemize}
    \item the auctions have completed AND 1 year has passed.
    \item the auctions have completed AND the operating system is at a point in its development where future changes can be approved or denied via Improvement Proposals approved by \$veKINO voters.
