...
Other nodes can now interact with the booted node.

The virtual machine is developed and distributed by Kinode DAO, which maintains a reference implementation. A formal specification of all aspects of the virtual machine is currently under development.
...
\footnote{The Wasm runtime is by a wide margin the most complex aspect of the system, and at least a dozen such runtimes exist today, written in multiple languages.
...
\begin{enumerate}
    ...
    to execute compiled processes which implement the Kinode WIT standard.
    \item Implementing the host functions, exposed to all processes, defined in Kinode WIT standard.
    \item Implementing the kernel API that allows processes with kernel-messaging capabilities to perform process management.
    \item Passing messages between all processes including to/from the kernel itself.
    \item Enforcing messaging capabilities.
\end{enumerate}


As of this writing, the microkernel runtime module in Kinode DAO's implementation of the OS fits into about 2,700 lines of Rust.
