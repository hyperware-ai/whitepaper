...
\footnote{Timeouts must be viewed as a lower bound, as in, the kernel will not return a timeout error for at least X seconds.
Depending on the implementation of the kernel, timeouts may occur at any time after X seconds have passed.}
...
\begin{enumerate}
    ...
    \item Voting on proposals to improve Kinode in a backwards-compatible way over time, while ensuring that adoption of the offchain software remains aligned with growth of the onchain protocol.
\end{enumerate}
