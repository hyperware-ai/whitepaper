...
The minumum registration duration is four weeks (one ``month'').
...
More details about registration can be found in the Hyperware Tokenomics document, published separately.
...
In \hyperref[fig:triangle]{Fig.~\ref{fig:triangle}}, Hyperware is presented as a triangle with Hypermap+HYPR and Hyperware OS above Hyperware Governance.
...
HYPR can be locked, producing voting power as a side effect, and allowing for registration to Hypermap entries as discussed in Section~\ref{sec:hyprregister}.
Voting power is related to the number of HYPR tokens locked and the duration they are locked for.
Voting power can be delegated.
More information about locking and registration can be found in the Hyperware Tokenomics document.
...
\textbf{Hyperware is not a purely onchain protocol}, however, and its governance must be executed in a decentralized manner for the network to be stable, neutral, and permissionless.
...
At some point, ownership of the Hypermap namespace, and in particular the TLZs, will be distributed enough that no single entity could disrupt the operation of the network as a whole by abusing ownership rights.
Since each TLZ can be the root of an entire namespace, the theoretical security requirements for continued operation of the network are 1 of N good actors.
...
Therefore, once enough TLZs are held by a diversity of immutable smart contracts, and possibly mutable contracts controlled by DAOs, the Hyperware governance mechanism will have succeeded in the first phase of its role.
...
These will be minted directly to contracts, including those already in use such as \verb|os|, \verb|dev|, and \verb|hypr|, those associated with an existing onchain identity primitive such as \verb|eth|, and anything else useful.
...
Development of the OS to the point of specification may take anywhere from 6 months to one year from the current implementation.
...
Hyperdrive will include a Governance Portal app, alongside the App Store and other utilites.
...
The governance protocol may do other work to increase the utility of the namespace and improve the functionality of the operating system, such as developing more Hypermap protocols like HNS and the App Store.
...
The initial deployment will take place on a single blockchain, but both volume of transactions and support for composition with other onchain protocols may inspire expansion to other chains.
...
However, making this sufficiently approachable for non-technical users has been a stumbling block for past instances of similarly decentralized software.
